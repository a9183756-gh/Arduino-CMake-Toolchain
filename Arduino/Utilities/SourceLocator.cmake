...
function(_find_sources _base_path _pattern _return_var)
	...
	set(_filtered_source_files)
	foreach(_source IN LISTS source_files)
		get_filename_component(_source_name "${_source}" NAME)
		if (NOT _source_name MATCHES "^\\.")
			list(APPEND _filtered_source_files "${_source}")
		endif()
	endforeach()

	set(${_return_var} "${_filtered_source_files}" PARENT_SCOPE)
...
function(find_source_files _base_path _return_var)
	...
	cmake_parse_arguments(parse_args "NO_ENABLE_LANGUAGE" "" "" ${ARGN})

	_find_sources("${_base_path}" "${ARDUINO_CMAKE_ASM_FILES_PATTERN}" sources
		${parse_args_UNPARSED_ARGUMENTS})
	if (sources)
		list(APPEND all_sources "${sources}")
		if (NOT parse_args_NO_ENABLE_LANGUAGE)
			enable_language(ASM)
		endif()
	endif()
	_find_sources("${_base_path}" "${ARDUINO_CMAKE_C_FILES_PATTERN}" sources
	${parse_args_UNPARSED_ARGUMENTS})
	if (sources)
		list(APPEND all_sources "${sources}")
		if (NOT parse_args_NO_ENABLE_LANGUAGE)
	...
	_find_sources("${_base_path}" "${ARDUINO_CMAKE_CXX_FILES_PATTERN}" sources
		${parse_args_UNPARSED_ARGUMENTS})
	if (sources)
		list(APPEND all_sources "${sources}")
		if (NOT parse_args_NO_ENABLE_LANGUAGE)
