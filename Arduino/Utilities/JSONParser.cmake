	...
	foreach(var ${token_list})
		...
		if (${json_parse_state} STREQUAL "VALUE")
			...
			elseif(var MATCHES "^-?[0-9]+")
				...
				if (NOT match)
					error_exit("@${curr_field}: Expected a value, but got \"${var}\"")
				endif()
		...
		elseif (${json_parse_state} STREQUAL "NAME1" OR
			...
			if (${json_parse_state} STREQUAL "NAME1" AND
				...
				string(REGEX MATCH "^\"(.*)\"$" match "${var}")
				if (NOT match)
					error_exit("@${curr_field}: Expected a name, but got \"${var}\"")
				endif()
		...
		elseif (${json_parse_state} STREQUAL "FIELD_SEP")
			...
			elseif (var STREQUAL "}")
				...
				error_exit("@${curr_field}: Expected `,` or `}`, but got \"${var}\"")
			endif()
		...
		elseif (${json_parse_state} STREQUAL "NAME_SEP")
			...
			if (var STREQUAL ":")
				...
				error_exit("@${curr_field}: Expected `:`, but got \"${var}\"")
			endif()
	
		elseif (${json_parse_state} STREQUAL "END")
	
			error_exit("@${curr_field}: Unexpected \"${var}\" after parsing completed")
...
function(json_get_value namespace path return_value)
	...
	if (NOT DEFINED "${namespace}.${path}")
		if (NOT parsed_args_QUIET AND "${parsed_args_DEFAULT}" STREQUAL "")
			error_exit("JSON path '${path}' in '${namespace}' invalid!!!")
		else()
...
function(json_get_list namespace pattern return_list)
	...
	foreach (_elem IN LISTS _variableNames)
		string_regex_replace_start("${namespace_regex}\\." "" _elem "${_elem}")
		list(APPEND _variableNames2 "${_elem}")
	endforeach()
	set("${return_list}" "${_variableNames2}" PARENT_SCOPE)
endfunction()
...
function(_json_leave_context)
	...
	elseif ("${${curr_field}/type}" STREQUAL "array") # Array
		...
		error_exit("@${curr_field} Assert. Script error? ${${curr_field}/type}")
	endif()
